        ...
        if ( rec?$host && rec?$user_agent && /crl\.microsoft\.com/ in rec$host &&
             /Microsoft-CryptoAPI\// in rec$user_agent )
                ...
                if ( rec$user_agent !in OS::crypto_api_mapping )
                        {
			found_os(rec$id$orig_h, "WINDOWS", "Unknown CryptoAPI Version", "WINDOWS", RequestInfo($ts=rec$ts, $user_agent=rec$user_agent, $host=rec$host, $uri=rec$uri));
                        }
                else
                        {
			...
			found_os(rec$id$orig_h, name, fmt ("%s.%s",result$version$major, result$version$minor), "WINDOWS", RequestInfo($ts=rec$ts, $user_agent=rec$user_agent, $host=rec$host, $uri=rec$uri));
                        }
                }
        }
